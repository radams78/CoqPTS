...
Notation "f =f g" := (funceq f g) (at level 50).
...
Definition id (X : Set) (x : X) := x.

Definition comp (X Y Z : Set) (g : Y -> Z) (f : X -> Y) (x : X) := g (f x).
Implicit Arguments comp [X Y Z].
Notation "f (O) g" := (comp f g) (at level 100).

Definition injective (X Y : Set) (f : X -> Y) :=
...
Definition bot (n : nat) : fin (S n) := None.
Implicit Arguments bot [n].
Definition up (n : nat) (x : fin n) : fin (S n) := Some x.
Implicit Arguments up [n].

Lemma up_wd : forall (n : nat) (x y : fin n), x = y -> up x = up y.
Proof.
intro n.
apply f_equal.
Qed.

Lemma up_inj : forall (n : nat) (x y : fin n), up x = up y -> x = y.
Proof.
intro n.
apply Some_inj with (X := fin n).
Qed.

Definition Sfun (m n : nat) (f : fin m -> fin n) (x :fin (S m)) : fin (S n) :=
  match x with
  | None => bot
  | Some y => up (f y)
  end.
...
Lemma Sfun_ext : forall (m n : nat) (rho sigma : fin m -> fin n), rho =f sigma -> Sfun rho =f Sfun sigma.
Proof.
unfold funceq.
destruct x; simpl.
...
Lemma Sfun_inj : forall (m n : nat) (rho : fin m -> fin n),
  injective rho -> injective (Sfun rho).
Proof.
...
Lemma Sfun_id : forall (n : nat), Sfun (id (fin n)) =f id _.
Proof.
unfold funceq.
destruct x; auto.
Qed.

Lemma Sfun_comp : forall (m n p : nat) (rho : fin m -> fin n) (sigma : fin n -> fin p),
